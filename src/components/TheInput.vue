<template>
  ...
  <form @submit.prevent="getUser" class="flex justify-between rounded-md gap-4 bg-[#1F2A48] p-4 w-1/2 mx-auto">
    <div class="flex items-center bg-transparent gap-4">
      ...
      <input v-model="search" class="bg-transparent pr-40 pl-4 py-2 placeholder-white" type="text" placeholder="Search Github username...">
    </div>
    <input class="bg-blue-500 text-white py-2 px-4 rounded-md cursor-pointer hover:bg-blue-600 active:bg-blue-600" type="submit" value="Search">
  </form>
  </div>
</template>
